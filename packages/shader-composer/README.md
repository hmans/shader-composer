...
Creating shaders from node graphs is fun -- users of engines like Unity or Unreal Engine do it all the time! Shader Composer aims at **providing a node-based shader composition experience from a code-first perspective**, designed for use with [Three.js].

If you've always liked the idea of composing shaders from node graphs, but secretly preferred doing it through code, then I am happy to admit that you are just like me, and that I have created Shader Composer for people like us. Let's go! 🚀
...
Shader Composer also has a couple of non-features:
...
This document is going to explain the basics of writing shaders using Shader Composer and developing your own nodes, but the best way to dive in probably is to check out one of the many great examples[^1].

- **[The official Shader Composer sandbox](https://codesandbox.io/s/Shader Composer-ts-y3h24k?file=/src/App.tsx)**. Fork it and go nuts!
- [Chunky Planet Geneerator](https://codesandbox.io/s/Shader Composer-planet-gkulwg?file=/src/App.tsx)
- [Dissolve Effect](https://codesandbox.io/s/Shader Composer-dissolve-ubzbxq?file=/src/App.tsx)
...
Shader Composer allows you to express your shader code as a tree of JavaScript objects, from which it will compile the GLSL for you. For example:
...
These objects are called **Nodes**, and every single one of them can be compiled into a shader program; but you typically start with a **Master Node**. Which Master you use entirely depends on how you intend to run the shader. Shader Composer currently provides Masters for use with Three.js' `ShaderMaterial` as well as [three-custom-shader-material]; more Masters may be added in the future.

> **Note**
>
> Shader Composer currently has a dependency to Three.js, but only really uses it for some type glue (eg. so you can pass a `THREE.Vector2` instance to set a `vec2` node.) It is very likely that at some point in the future, Shader Composer can be used outside of Three.js, too; PRs welcome!
...
Everything in Shader Composer is expressed as a **Node**. Some basic facts about nodes:
...
There you go, you've built your first animated shader with Shader Composer! Now go crazy!
